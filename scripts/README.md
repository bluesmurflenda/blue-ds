...
## R. 화면 검사 — `docs/` 를 실제로 띄워서 본다

근거: **다른 검사는 전부 「값이 맞나」를 본다.** 값이 다 맞아도 화면은 깨질 수 있다.
실제로 세 건이 그렇게 통과했다 — 카드 내용이 카드보다 넓어 잘렸고, 목록 행 제목이
폭 0 이 되어 글자가 통째로 사라졌고, 다크 모드에서 검은 배경에 검은 글자가 나왔다.

**픽셀을 비교하지 않는다.** 계산된 스타일과 크기만 본다. 기준 이미지를 저장할 필요가 없고,
폰트 렌더링 차이로 인한 헛경보가 없다.

문서 사이트 전 페이지를 **좁은 폭·넓은 폭 × 라이트·다크** 네 조합으로 연다.
한 폭만 보면 「창 폭에 따라 칸 수가 흔들리는」 문제가 안 잡힌다.

| 항목 | 보는 것 | 판정 |
| --- | --- | --- |
| R1 | 페이지가 가로로 넘치나 | 실패 |
| R2 | 넘침 숨김 요소 안에서 내용이 잘리나 — 말줄임은 뺀다 | 실패 |
| R3 | 절대배치 요소가 부모 밖으로 나갔나 | 실패 |
| R4 | placeholder 가 칸에 안 들어가나 — 글자 폭을 직접 잰다 | 실패 |
| R5 | 글자 대비가 기준(4.5·큰 글자 3.0)에 못 미치나 — 비활성은 뺀다 | 경고 |
| R6 | 나열 격자의 열 수가 사이즈 축 개수와 다른가 | 경고 |

**R5·R6 이 경고인 이유** — 색을 바꾸면 그 토큰을 쓰는 다른 자리가 같이 바뀌고,
칸 수는 무엇을 비교시킬지에 달렸다. **사람이 정할 판단이 섞여 있어 커밋을 막지 않는다.**
CI 에서 조이려면 `--strict` 를 준다.

**일부러 그런 것은 `check-visual.mjs` 의 `EXPECT` 에 이유와 함께 넣는다.**
이유 없이 늘면 검사가 죽는다.

**필요한 것** — `playwright` 와 크로미움. 없으면 스크립트가 설치 명령을 알려 주고 멈춘다.
