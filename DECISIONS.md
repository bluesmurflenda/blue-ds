...
## ADR-038 — 초점 표시는 한 가지다: 안쪽 테두리 + 바깥 헤일로

초점 표시는 폼 요소든 버튼이든 같은 모양을 쓴다. 두 가지를 만들지 않는다.

- **안쪽 테두리 1px** — 이것이 대비를 진다.
- **바깥 헤일로 2px** — 장식이다. 이것만으로는 기준을 못 넘는다.
- **둘 사이에 간격을 두지 않는다.**

### 테두리 색은 그 요소의 글자색을 쓴다

버튼 계열의 초점 테두리는 자기 글자색이다(CSS 에서는 `currentColor`).
글자색은 원래부터 자기 배경 위에서 읽히도록 정해진 색이라 대비가 자동으로 보장되고,
모드가 바뀌면 같이 따라간다. 초점 전용 색을 변형마다 새로 만들지 않는다.

**초점 표시에 브랜드색 하나를 고정하면 안 된다.** 버튼 몸통 색이 변형마다 다르기 때문에
어떤 색을 고르든 그 색과 비슷한 몸통에서는 보이지 않는다. 선을 굵게 해도, 헤일로를
진하게 해도 달라지지 않는다 — 색이 같으면 안 보인다.

폼 요소는 예외다. 몸통이 항상 바탕색이므로 초점 테두리 색을 고정해 쓸 수 있고,
검증 상태(정상·유효·오류)에 따라 테두리와 헤일로 색이 함께 바뀐다.
검증 축은 폼 요소에만 있다 — 버튼 계열에 넘어오면 빌드를 멈춘다.

### pressed 에는 초점 표시가 없다

눌린 상태는 배경색으로 말한다. 초점 표시는 focus 에만 둔다.

### 마우스로 눌러도 나온다

`:focus-visible` 이 아니라 `:focus` 를 쓴다. 키보드로 왔을 때만 보이면
문서·검수 화면에서 상태를 확인할 방법이 없다.

### 라디오의 :indeterminate 는 체크박스의 중간 상태가 아니다

라디오에서 이 선택자는 「그룹에 선택된 것이 없음」을 뜻한다.
두 가지를 한 선택자에 묶으면 아직 아무것도 고르지 않은 라디오가 선택된 모양으로 그려진다.
중간 상태를 다루는 선택자는 반드시 체크박스 아래로 좁힌다.

### 예외 세 가지

- **배경이 투명한 변형** — Figma 의 퍼짐 그림자는 불투명한 채움이 있어야 그려진다.
  이 변형들은 테두리만 남고 헤일로가 빠진다. 테두리가 대비를 지므로 그대로 둔다.
  그리라고 투명 채움을 끼워 넣지 않는다 — 그려지지도 않으면서 바인딩 없는 색만 남는다.
- **텍스트 링크** — 글자 크기에 딱 맞는 틀이라 안쪽 테두리가 글자를 문다.
  Figma 에서는 바깥 테두리로 두고, 코드에서는 링을 2px 띄운다.
- **아코디언 항목** — 항목이 서로 붙어 있어 바깥에 링을 그릴 자리가 없다.
  링을 안쪽에만 그리고 헤일로를 뺀다. 자리를 만들려고 잘라내기를 넓히면 둥근 모서리가 사라진다.
  또 inset 그림자는 자식 배경에 덮이므로 outline 으로 그린다.

### 고칠 때 확인하는 것

초점 표시를 바꿨으면 **값이 들어갔는지가 아니라 대비를 잰다.**
테두리 색과 몸통 색을 두 모드에서 각각 풀어 3:1 을 넘는지 계산한다.
값만 되읽는 검사는 색이 같거나 그림자가 안 그려져도 통과한다.