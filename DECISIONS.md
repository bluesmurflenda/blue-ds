# BLUESOFT 디자인시스템 — 결정 기록

**append-only.** 승인된 기록을 나중에 편집하지 않는다.
결정이 바뀌면 원본을 `Superseded` 로 표시하고 새 기록을 쓴 뒤 둘을 연결한다.

## 형식

```
## ADR-000 · 제목
상태: Proposed | Accepted | Superseded
확신도: 높음 | 중 | 낮음
날짜: YYYY-MM-DD

### 맥락      왜 결정이 필요했는가
### 결정      무엇으로 정했는가
### 근거      왜 그렇게 정했는가
### 뒤집으려면 되돌리는 방법과 예상 작업량
```

## 값에 대한 주의

**ADR 안의 수치는 결정 당시의 실측값이다.** 현재 상태가 아니다.
"45변형" "500개" 같은 숫자는 **왜 그렇게 판단했는지**를 남기려고 적은 것이다.

현재 값이 필요하면 Figma MCP 로 조회하거나 검사 스크립트를 돌린다.

## 운영

- Claude 앱이 **전부 `Proposed` 로 채운다.** 사람에게 미세 판단을 위임하지 않는다
- 사람은 **확신도 "낮음"만** 훑고 틀린 것만 뒤집는다
- `Accepted` 된 것은 **다시 묻지 않는다**
- 커서는 이 문서를 읽고 따른다. **없는 판단이 필요하면 멈추고 보고한다**

| 확신도 | 의미 | 사람이 볼 필요 |
|---|---|---|
| 높음 | 실측 근거가 명확하고 대안이 없다 | 없음 |
| 중 | 두 안이 있지만 한쪽이 명확히 낫다 | 없음 |
| 낮음 | 취향·브랜드 판단이 섞였다 | **있음** |

## 새 ADR 을 쓸 기준

**두 조건을 다 만족할 때만 ADR 이다.** (1) 되돌리는 비용이 크다 (2) 구조 자체를 바꾼다.
하나만 해당하면 ADR 이 아니라 **커밋 메시지**로 충분하다 — 그 자리에서 왜인지 적고 끝낸다.
ADR 을 쓸지 매번 고민하는 것 자체가 시간을 쓰는 일이다.

| 예 | ADR? |
| --- | --- |
| 배포를 복붙에서 npm 패키지로 바꾼다 | O — 되돌리기 어렵고 구조를 바꾼다 |
| `_input.scss` radius 가 버튼 토큰 대신 `field/radius` 를 참조한다 | X — 커밋 메시지. 파일 하나, 값 하나 |
| 인라인 hex 를 시맨틱 토큰으로 바꾼다 | X — 커밋 메시지. S2 가 검사로 대신한다 |
| Wide 구간 미디어쿼리를 없앤다(ADR-026) | O — 컴포넌트 전체의 반응형 방식을 바꾼다 |

**망설여지면 커밋 메시지로 시작한다.** 되돌리기 정말 어려운 결정이었다고 나중에 드러나면
그때 ADR 로 승격한다 — ADR 로 시작해 놓고 안 쓰는 것보다 이 방향이 되돌리기 쉽다.

기존 ADR 은 다시 정리하지 않는다 — 그 자체가 문서 작업이다. 이 기준은 다음 결정부터 적용한다.

---

## ADR-001 · 중성색은 neutral 하나로 통일한다

상태: Accepted 확신도: 높음 날짜: 2026-08-29

### 맥락
중성색 계열이 `neutral` 과 `slate` 두 개로 갈려 있었다.
`slate` 는 푸른 기가 도는 회색이라 같은 화면에 두 계열이 섞였다.
다크모드에서 테이블 헤더가 남색으로 뜨는 원인이었다.

### 결정
`slate/*` 를 전부 `neutral/*` 같은 단계로 교체하고 프리미티브를 삭제했다.

### 근거
중성색이 두 계열일 이유가 없다. `slate` 는 외부 라이브러리에서 넘어온 흔적이었다.
라이트모드 색이 미세하게 바뀌지만(`#f8fafc` → `#fafafa`) 육안 차이는 거의 없다.

### 뒤집으려면
`Primitive` 에 `slate/50~950` 을 다시 만들고 해당 토큰을 재바인딩. 반나절.

---

## ADR-002 · Theme=dark 는 다크모드가 아니다

상태: Accepted 확신도: 높음 날짜: 2026-08-29

### 맥락
`Header` · `Tooltip` · nav 계열의 `Theme` 축과 다크모드가 이름이 같아 혼동됐다.
기계적 반전 규칙을 적용했다가 "어두운 스타일" 툴팁이 다크모드에서 흰색이 됐다.

### 결정
`Theme` 축은 **"스타일"** 로 정의한다. `*/dark/*` · `*/light/*` 토큰은 다크모드에서 반전하지 않는다.
축 이름은 `light|dark` 로 유지한다.

### 근거
축 이름을 `default|inverse` 로 바꾸는 것을 검토했으나, `light|dark` 를 쓰는 세트가 4개이고
인스턴스가 많아 변경 위험이 실익보다 크다.
토큰 값으로 동작을 정의하면 축 이름이 같아도 혼동이 없다.

### 뒤집으려면
4개 세트의 축 값을 동시에 변경 + 인스턴스 재확인. 하루 이상. 오버라이드 유실 위험.

---

## ADR-003 · 컴포넌트 토큰은 시맨틱을 경유한다

상태: Accepted 확신도: 높음 날짜: 2026-08-29

### 맥락
컴포넌트 토큰 상당수가 프리미티브를 직접 참조했다.
`surface/default` 를 바꿔도 카드·버튼이 따라오지 않았다.

### 결정
같은 값의 시맨틱 토큰이 있으면 그것을 경유한다.
시맨틱에 대응이 없는 것(의미색·브랜드 자산·컴포넌트 고유 톤)만 프리미티브 직접 참조를 허용한다.

### 근거
계층이 끊기면 시맨틱 계층의 존재 이유가 없어진다.
경유 시 값이 바뀌지 않으므로 시각적 위험이 없다.

### 뒤집으려면
개별 토큰을 프리미티브로 되돌리면 된다. 건별 5분.

---

## ADR-004 · 아이콘 전용 토큰을 둔다

상태: Accepted 확신도: 중 날짜: 2026-08-29

### 맥락
아이콘 색이 `neutral/600` 프리미티브를 직접 참조해 다크모드에서 안 바뀌었다.
`text/*` 를 재사용할지 전용 토큰을 만들지 판단이 필요했다.

### 결정
`icon/primary` · `subtle` · `strong` · `disabled` · `brand` · `inverse` 를 신설한다.
이름 형식은 `text/*` 와 맞춘다(접미사 `fg` 없음).

### 근거
**아이콘은 UI 그래픽이라 대비 기준이 3:1 이고 텍스트는 4.5:1 이다.**
`text/*` 를 재사용하면 기준이 섞여 아이콘만 조정할 수 없다.

### 뒤집으려면
`icon/*` 을 삭제하고 `text/*` 로 재바인딩. 반나절.

---

## ADR-005 · Icon Set 마스터는 프리미티브를 직접 참조한다

상태: Accepted 확신도: 높음 날짜: 2026-08-29

### 맥락
Icon Set 파일의 아이콘 마스터가 `neutral/900` · `sky/500` 프리미티브를 물고 있다.
ADR-003 과 어긋나 보인다.

### 결정
**Icon Set 마스터는 그대로 둔다.** 색은 사용처에서 오버라이드한다.

### 근거
마스터가 시맨틱을 물면 `Icon Set → Theme` 참조가 생겨 의존 방향이 역전된다.
지금은 `Icon Set → DS Master` 단방향이다.

### 뒤집으려면
순환 의존이 생기므로 뒤집지 않는다.

---

## ADR-006 · container/width 하나만 남긴다

상태: Accepted 확신도: 중 날짜: 2026-08-29

### 맥락
`container/width`(브레이크포인트별 실제 폭)와 `container/max-width`(1200 고정)가
같은 자리에 섞여 쓰여 Laptop·Mobile 컨테이너가 1200 으로 잡혀 있었다.

### 결정
`container/width` 만 남기고 `container/max-width` 를 삭제했다.
CSS 는 `width:100%` + `max-width` 하드값 + `margin-inline:auto` 구조를 쓴다.

### 근거
Figma 는 `width:100%` 를 표현할 수 없어 브레이크포인트별 실제 폭이 필요하다.
CSS 의 `max-width` 는 1200 하나뿐이라 토큰이 필요 없다.
`max-width` 로 통일하면 Mobile 시안이 1200 이 되어 깨진다.

### 뒤집으려면
`Breakpoint` 에 `container/max-width` 를 1200 고정으로 다시 만든다. 30분.

---

## ADR-007 · Table/Cell 슬롯 이름을 유지한다

상태: Accepted 확신도: 중 날짜: 2026-08-29

### 맥락
슬롯 이름(`Leading` · `Chip` · `Input` · `Action`)이 실제 용도와 어긋난다.
`Chip` 슬롯의 상당수가 Chip 이 아니라 Avatar Group 이다.

### 결정
**이름을 유지한다.** 슬롯 선택은 이름이 아니라 사이징(FIXED / HUG)으로 판단한다.
대개편 시 `Lead` · `Content` · `Trailing` 으로 함께 바꾼다.

### 근거
인스턴스가 500개를 넘어 이름 변경 시 오버라이드 유실 위험이 크다.
얻는 것이 이름의 정확성뿐이다.

### 뒤집으려면
Table/Cell 대개편과 함께. 인스턴스 전수 재확인 필요.

---

## ADR-008 · Table/Cell 높이는 min-height 다

상태: Accepted 확신도: 높음 날짜: 2026-08-29

### 맥락
셀 높이가 고정이라 긴 텍스트가 잘렸다.
`padding-y` 토큰이 있었으나 실제 컴포넌트는 `padding-y: 0` 이었다.

### 결정
`counterAxisSizingMode: AUTO` + `minHeight` 로 바꾸고 `min-height` 토큰을 바인딩했다.
`padding-y` 토큰은 삭제했다. 여백은 세로 중앙 정렬이 만든다.

### 근거
한 줄일 때는 이전과 같고 여러 줄일 때만 늘어난다.
`padding-y` 를 함께 쓰면 여러 줄에서 높이가 과도해진다.

### 뒤집으려면
`counterAxisSizingMode` 를 FIXED 로 되돌린다. 30분. 다만 텍스트 잘림이 재발한다.

---

## ADR-009 · Button Size 는 xs 를 포함한다

상태: **Superseded → ADR-015** 확신도: 낮음 날짜: 2026-08-29

### 맥락
`Button` 에 `Size=xs` 가 45변형 있으나 인스턴스 사용처가 0건이다.
코드에 포함할지 판단이 필요했다.

### 결정
코드에 포함한다. 5단계(xs·sm·md·lg·xl)로 구현한다.

### 근거
`Table/Cell` 의 작은 사이즈처럼 조밀한 자리에서 `sm` 이 큰 상황이 나올 수 있다.

### 뒤집힌 이유
"나올 수 있다"는 가정이었고 실제 사용처가 없었다. → **ADR-015**

---


## ADR-010 · Variant 별 토큰은 독립시킨다

상태: Accepted 확신도: 높음 날짜: 2026-08-29

### 맥락
`tabs/pill` 이 `tabs/underline` 토큰을 참조해 두 Variant 가 묶여 있었다.
`underline` 을 바꾸면 `pill` 도 바뀐다.

### 결정
값이 같아도 Variant 별로 독립 토큰을 만든다.
컴포넌트 토큰이 같은 그룹의 다른 하위그룹을 참조하지 않는다.

### 근거
Variant 는 독립적으로 조정할 수 있어야 한다.
토큰 개수가 늘지만 별칭이라 유지 비용이 낮다.

### 뒤집으려면
중복 토큰을 삭제하고 공용 토큰으로 재바인딩. 건별 10분.

---

## ADR-011 · 컴포넌트 세트 배경은 surface/default 다

상태: Accepted 확신도: 중 날짜: 2026-08-29

### 맥락
세트 프레임 배경이 `white` 고정이라 페이지를 `Theme=Dark` 로 바꿔도
배경이 흰색으로 남아 다크 변형을 판단하기 어려웠다.

### 결정
`surface/default` 로 바꾼다. 모드를 따라간다.

### 근거
컴포넌트 배경이 `surface/default` 인 경우가 많아, 세트 배경도 같으면
그 컴포넌트가 실제로 어떻게 보이는지 정확히 판단된다.

### 뒤집으려면
`white` 로 되돌린다. 10분.

---

## ADR-012 · avatar/c/* 는 Theme 에 직접값으로 둔다

상태: Accepted 확신도: 중 날짜: 2026-08-29

### 맥락
아바타 배경 파스텔 13색이 프리미티브 램프에 없어 별칭을 걸 대상이 없다.
`Theme` 계층에 직접값이 있는 것은 원칙 위반으로 보인다.

### 결정
`Theme` 에 직접값으로 유지한다. 검사 스크립트는 예외로 통과시킨다.

### 근거
아바타 전용 색이고 13개뿐이라 프리미티브로 올려도 재사용처가 없다.
색상군이 하나 늘어나는 비용이 더 크다.

### 뒤집으려면
`Primitive` 에 `avatar-c/01~13` 을 추가하고 `Theme` 이 별칭으로 참조. 30분.

---

## ADR-013 · 문서에 값·수치를 넣지 않는다

상태: Accepted 확신도: 높음 날짜: 2026-08-29

### 맥락
문서에 값을 적으면 Figma 가 바뀔 때마다 낡는다.
낡은 값이 코드에 들어가 다크모드가 깨진 사고가 있었다.
문서를 검토할 때마다 새 오류가 나와 끝나지 않았다.

### 결정
문서에는 **방침만** 쓴다. 값·수치·API 동작 단정을 넣지 않는다.
값이 필요하면 Figma MCP 로 조회하고, 대조는 검사 스크립트가 한다.

### 근거
방침은 사람이 정한 것이라 실측 대상이 아니다 — 검토할 게 없다.
값은 스크립트가 실행 시점에 읽으므로 항상 맞다.

### 뒤집으려면
뒤집지 않는다. 이 결정이 반복 수정을 끊는 근거다.

---

## ADR-014 · 검사 항목은 누적한다

상태: **Superseded → ADR-035** 확신도: 높음 날짜: 2026-08-29

### 맥락
"완벽해질 때까지 검토"는 끝나지 않는다. 검토 각도가 무한하기 때문이다.

### 결정
목표를 **회귀 방지**로 둔다.
발견한 문제 유형을 검사 항목으로 만들고, 새 문제가 나오면 항목을 추가한다.
**항목은 줄지 않는다.**

### 근거
스크립트가 틀리면 그 스크립트를 고치면 되고, 고치면 다시는 안 틀린다.
사람이 매번 새로 검토하면 매번 다르게 틀린다.

### 뒤집으려면
뒤집지 않는다.

---

## ADR-015 · Button Size 는 xs 를 제외한다

상태: Accepted 확신도: 높음 날짜: 2026-08-29
대체: ADR-009

### 맥락
ADR-009 에서 `xs` 를 코드에 포함하기로 했으나, 근거가 "필요해질 수 있다"는 가정뿐이었다.
실제 인스턴스 사용처는 0건이다.

### 결정
**코드에서 제외한다.** `$button-sizes` Map 은 4단계(sm·md·lg·xl)로 만든다.
`button/radius-xs` 도 CSS 변수로 내보내지 않는다.
`Icon Button` 은 원래 Figma 에 xs 가 없어 동일하게 4단계다.

**Figma 의 45변형은 삭제하지 않고 유지한다.**

### 근거
쓰이지 않는 45변형을 코드로 만들면 유지 대상만 늘어난다.
Figma 에 남아 있으므로 필요해지면 코드만 추가하면 된다 — 추가 비용이 낮다.

### 뒤집으려면
Map 에 xs 항목을 추가한다. 10분.

```scss
xs: (height: 28px, pad: var(--button-padding-x-xs), type: body-2xs, radius: var(--button-radius-xs))
```

---

---

## ADR-016 · DESIGN-SYSTEM.md 를 만들지 않는다

상태: Accepted 확신도: 높음 날짜: 2026-08-29

### 맥락
초기 계획은 문서 4개였다 — `CLAUDE.md` · `DESIGN-SYSTEM.md` · `DECISIONS.md` · `PROGRESS.md`.
`DESIGN-SYSTEM.md` 는 "구조·예외·근거"를 담을 예정이었다.

### 결정
**만들지 않는다.** 그 내용은 `CLAUDE.md` 에 흡수한다.

### 근거
ADR-013 으로 "문서에 값·수치를 넣지 않는다"를 정한 뒤,
`DESIGN-SYSTEM.md` 에 남을 것이 **방침뿐**이 되었다. 방침은 `CLAUDE.md` 의 역할이다.
문서를 나누면 같은 주제가 두 곳에 흩어져 "한 사실은 한 파일에" 를 어긴다.

컴포넌트별 상세는 Figma MCP 조회로 대체한다 — 문서보다 정확하고 낡지 않는다.

### 뒤집으려면
`CLAUDE.md` 에서 구조 관련 장(1·2·4·6·7장)을 분리한다. 반나절.
다만 분리 기준이 모호해 다시 겹칠 위험이 있다.

---

## ADR-017 · 변수 메타데이터 검사는 스크립트가 아니라 MCP 수동 실행이다

상태: Accepted 확신도: 높음 날짜: 2026-08-30

### 맥락

`check-ds.mjs`(현 `check-nodes.mjs`)가 `GET /v1/files/{key}/variables/local` 로 변수 컬렉션·값을
받아 D1~D13 전부를 스크립트화할 계획이었다. 실제로 호출해보니 401 — Figma Variables REST API는
Enterprise 플랜 전용이고 이 프로젝트의 토큰은 해당 플랜이 아니다.

`GET /v1/files/{key}`(노드 트리) 로 대체 가능한지 실측했다. 컴포넌트 노드의 `boundVariables`는
`{"type":"VARIABLE_ALIAS","id":"VariableID:997:12"}` 형태로 **변수 id만** 주고 이름·컬렉션·값을
주지 않는다 — 이름 해석에 반드시 변수 목록이 필요한데 REST로는 얻을 수 없다.

### 결정

**노드 트리만으로 되는 검사(D1·D2·D10)만 스크립트(`check-nodes.mjs`)로 남긴다.**
변수 이름·값·scopes·description·별칭이 필요한 검사(D3~D9·D11~D13)는 Figma MCP로 대화 중
수동 실행한다. 단, id→이름 해석용으로 `figma/tokens.ids.json`(변수 id→이름·컬렉션 맵, MCP로
추출해 커밋)을 별도로 둬서 D1이 "이 프리미티브가 어떤 이름인지"는 알 수 있게 한다.

### 근거

REST 제약은 플랜을 바꾸지 않는 한 우회할 수 없는 사실이다. MCP(Plugin API)는 파일을 열어야
실행되므로 CI에 못 태우지만, 애초에 "V군은 Figma를 수정한 뒤 퍼블리시 전에" 도는 것이라
코드 작업 세션과는 실행 시점이 겹치지 않는다 — CI 부재가 실질적 손실이 아니다.

### 뒤집으려면

Figma 플랜을 Enterprise로 올리면 `/variables/local`을 다시 쓸 수 있다. 그 전까지는
`figma/tokens.ids.json` 갱신 절차(컬렉션 바뀔 때마다 MCP 재추출)를 유지한다.

---

## ADR-018 · 프리미티브 네이밍은 `color-` 접두사를 버리고 Figma 이름을 그대로 쓴다

상태: Accepted 확신도: 높음 날짜: 2026-08-30

### 맥락

`tokens/_primitive.scss`의 색상 변수는 전부 `--color-blue-500` 처럼 `color-` 접두사가 있다.
Figma Primitive 컬렉션의 실제 변수명은 `blue/500`(접두사 없음)이고, `codeSyntax.WEB`도
이미 `var(--blue-500)`으로 채워져 있다 — 코드와 Figma Dev Mode 표시가 서로 다르다(`check:tokens`
S2가 이 불일치를 157건의 "CSS 전용" + 491건의 "Figma 전용"으로 잡아냈다. 대부분 이 접두사 하나 때문).

### 결정

**`color-` 접두사를 제거하고 Figma 변수명을 그대로 쓴다.** `neutral/900` → `--neutral-900`
(현재 `--color-neutral-900`). `$color-*` SCSS 별칭과 이를 참조하는 컴포넌트 파일도 함께 정리한다.
**1단계(`tokens/` 재작성)에서 실행한다** — 지금 바로 리네임하지 않는다.

### 근거

Figma Dev Mode가 보여주는 이름과 코드의 변수 이름이 같아야 디자이너·개발자가 서로 다른 이름을
번역할 필요가 없다. `codeSyntax`가 이미 접두사 없는 형태로 채워져 있으므로, 코드 쪽을 거기에
맞추는 게 Figma를 코드에 맞춰 바꾸는 것보다 근거가 명확하다(Figma가 원본).

### 뒤집으려면

`$color-*` 별칭을 되살리고 컴포넌트 참조를 원복. 참조하는 곳이 많아(1단계 전체 파일) 반나절 이상.

---

## ADR-019 · D1(컬러 프리미티브 직접 참조) 예외 2건 — Checkbox 유효성 색·Avatar Online 링

상태: **Superseded — 예외가 없어졌다** 확신도: 높음 날짜: 2026-08-30

**2026-09-14 확인: `scripts/lib/allowlist.mjs` 의 `D1_NODE_EXCEPTIONS` 가 빈 배열이다.**
이 기록이 등록하기로 한 예외 2건이 코드에 없다. 아래는 그 시점의 판단으로 남긴다.

### 맥락

`check-nodes.mjs` D1이 Figma 라이브에서 23건을 찾았다 — `_Checkbox base`의 validation
링(valid=`green/600`·`green/700`, invalid=`red/500`·`red/600`)과 `Avatar`의 `Online`
표시 링(`white`)이 시맨틱 토큰 없이 프리미티브를 직접 바인딩하고 있었다.

### 결정

**둘 다 의도된 것으로 보고 allowlist 예외로 처리한다.** 전역 허용이 아니라
**컴포넌트 이름 + 프리미티브 이름 쌍으로 좁혀서** 등록한다(`scripts/lib/allowlist.mjs`
`D1_NODE_EXCEPTIONS`) — `white`를 전역으로 풀면 D1이 원래 잡으려던 회귀(체크박스·입력·
페이지네이션·모달 배경이 `white`를 직접 물어 다크모드에서 흰 판이 된 사고, D1 "나온 배경")를
다시 놓친다. `_Checkbox base`·`Avatar` 두 컴포넌트 안에서만 예외가 적용되고, 다른 컴포넌트가
같은 프리미티브를 직접 물면 여전히 FAIL이다.

### 근거

유효성 표시색(성공=초록/실패=빨강)과 온라인 표시(흰 테두리)는 `FIGMA.md` 「토큰 계층」의 "의미색"·
"유채색 배경 위 글자(장식 요소)"에 해당하고, 대응하는 시맨틱 토큰이 아직 없다.

### 뒤집으려면

해당 컴포넌트에 전용 시맨틱 토큰(예: `choice/validation-ring-valid` 등)을 만들고
`D1_NODE_EXCEPTIONS`에서 항목을 지운다. 건당 30분.

---

## ADR-020 · stylelint 를 도입하지 않는다

상태: **Superseded → ADR-036** 확신도: 높음 날짜: 2026-08-31

### 맥락

S6(중첩 깊이)·S7(BEM 위반) 강제 수단으로 stylelint 를 쓰기로 문서(`scripts/README.md`)에
적었고, `check-tokens.mjs`가 `stylelint-config-standard-scss` 기반 `.stylelintrc.json`을
`max-nesting-depth`·`selector-max-id` 두 규칙만 걸러 쓰는 방식으로 구현돼 있었다. devDependencies
에 `stylelint`가 있었지만 독립 실행 스크립트(`npm run lint`)는 없었다.

### 결정

**도입하지 않는다.** `check-tokens.mjs`가 SCSS 소스·컴파일된 CSS를 직접 스캔해 S6·S7b를
검사하도록 재작성한다(중괄호 균형 스캐너 + postcss 파싱, S5·S7a와 같은 방식). `stylelint`·
`stylelint-config-standard-scss`를 devDependencies에서 제거하고 `.stylelintrc.json`을
삭제한다. S7a·S7b가 쓰는 CSS 파싱은 `postcss`(stylelint의 전이 의존성으로만 설치돼 있던 것)를
독립 devDependency로 등록해 유지한다.

### 근거

`npm run lint`(전체 규칙)를 실제로 돌려보니 332건이 나왔는데, 우리가 원래 강제하려던 두
규칙(`max-nesting-depth`·`selector-max-id`)의 위반은 **0건**이었다 — 이미
`check-tokens.mjs`가 걸러서 통과 확인 중이었다. 나머지는:
- 102건 — `selector-class-pattern`이 이 프로젝트의 BEM `__`/`--` 표기를 kebab-case 위반으로
  오탐(`.tabs-item__content` 등 정상 클래스가 전부 걸림).
- 230건 — 빈 줄 삽입 규칙 5종(134건)·색상 표기법 규칙 4종(53건) 등 이 프로젝트가 정한 규칙과
  무관한 포매팅·스타일 취향.

전체 규칙 세트를 `check`에 넣으면 항상 FAIL하는 게이트가 되고, 필요한 2규칙만 쓰려면
`selector-class-pattern`을 BEM 패턴으로 재정의하고 나머지 규칙을 하나씩 꺼야 한다 — 그
설정을 만들고 유지하는 비용이, 이미 `check-tokens.mjs`로 통과 확인 중인 2규칙을 위해 낼
가치보다 크다.

### 뒤집으려면

`.stylelintrc.json`을 새로 만들어 `selector-class-pattern`을 BEM 패턴(`/^[a-z0-9-]+(__[a-z0-9-]+)?(--[a-z0-9-]+)?$/`
류)으로 지정하고, 나머지 무관한 규칙을 개별 비활성화한다. `check-tokens.mjs`의 S6/S7b를
`stylelint.lint()` 호출로 되돌린다. 반나절.

---

## ADR-021 · scopes 조이기 기준

상태: Accepted 확신도: 높음 날짜: 2026-08-31

### 맥락

Theme 컬렉션 토큰 다수가 `scopes: ALL_SCOPES`로 남아 있었다 — 어떤 속성에든 바인딩할 수
있다는 뜻이라, Figma 에서 변수를 고를 때 엉뚱한 자리(예: 배경색 자리에 텍스트 전용 토큰)가
후보로 잘못 뜰 여지가 컸다.

### 결정

**역할별로 좁힌다.** 램프(brand·accent 등 원시 색 단계)는 4개 scope 전부 유지, 배경은
`FRAME_FILL`+`SHAPE_FILL`, 테두리는 `STROKE_COLOR`, 전경(텍스트·아이콘)은 `TEXT_FILL`+
`SHAPE_FILL`로 좁힌다. Theme 77건을 이 기준으로 조였다 — 충돌 0건 확인.

**Primitive 189건은 조이지 않는다.** 프리미티브는 정의상 어디든 쓰일 수 있어야 하는
원시값 계층이라 scope 를 좁히는 것 자체가 목적에 안 맞는다.

**예외**: `surface/default`는 실사용이 넓어 4개 전부 유지(strokes 314곳·TEXT 2곳).
`border/default`는 `STROKE_COLOR`에 `SHAPE_FILL` 하나를 더했다(구분선 도형 채움 45곳).
순수 border 토큰 27건은 `STROKE_COLOR` 1개다.

### 근거

`ALL_SCOPES`는 후보가 너무 넓어 잘못 선택되기 쉽다 — scope 를 역할에 맞게 좁히면 Figma
속성 패널에서 그 자리에 맞는 토큰만 후보로 뜬다.

**다만 조인 뒤 실사용을 반드시 대조해야 한다** — `surface/default`가 테두리 자리에 314곳
쓰이는 것처럼, scope 를 좁히면서 그 변수가 실제로 걸쳐 쓰이던 속성을 놓치면 그 자리에서
후보 목록에서 사라져 버린다. 이번엔 대조 후 충돌 0건을 확인했다.

### 뒤집으려면

해당 토큰의 `scopes`를 `ALL_SCOPES`로 되돌린다. 건별 1분.

---

## ADR-022 · 아이콘 sprite 의 fill 을 빌드 산출물 단계에서 currentColor 로 치환한다

상태: Accepted 확신도: 높음 날짜: 2026-08-31

### 맥락

Alert 대조 작업 중 아이콘·닫기 버튼이 색상(brand/success/warning/...)과 무관하게 항상 회색으로
렌더링되는 걸 발견했다. 원인을 추적하니 `icons/raw/base/*.svg`(개별 원본, 293개)의 모든
`<path>`가 Figma export 시점 리터럴 `fill="#525252"`(우연히 `icon/primary` 라이트값과 일치)를
가지고 있고, 이 원본들이 조립된 `icons/sprite.svg`(1136개 `<symbol>`, 저장소 루트 — `docs/assets/`
사본이 아니라 원본)도 그 값을 그대로 물고 있었다.

`.icon` 컨테이너에 `fill: currentColor`를 CSS 로 걸어도 소용없다 — `<use>` 가 참조하는
`<symbol>` 내부 `<path>` 자체에 이미 지정값(`fill` 속성)이 있으면 조상의 상속값이 적용되지
않는다(SVG/CSS 명세: 요소 자신의 지정값이 상속보다 우선하고, `<use>` 참조 내용은 바깥 문서의
셀렉터로 직접 선택할 수 없다). 즉 SVG 소스 자체를 고쳐야 하는 문제였다.

`icons/sprite.svg`는 이 저장소(마스터 라이브러리)가 SI 프로젝트마다 복붙해 쓰는 산출물의
일부다(`README.md`: "Figma DS Master를 기준으로 SI 프로젝트마다 복붙해 쓰는 master SCSS").
`docs/` 는 `scripts/sync-docs-assets.mjs`가 이 파일을 그대로 복사한 미러일 뿐이라, docs 복사
단계에서만 치환하면 docs 사이트만 고쳐지고 실제로 이 파일을 가져다 쓰는 프로젝트들은 그대로
회색 버그를 물려받는다.

### 결정

`icons/sprite.svg`(저장소 루트, 배포 산출물) 자체를 새 스크립트로 치환한다.
`icons/raw/base/*.svg`(개별 원본)는 건드리지 않는다.

```
scripts/fix-icon-sprite-fill.mjs   fill="#525252" → fill="currentColor" (정확히 이 문자열만)
npm run fix:icon-sprite            로 실행
```

이 저장소엔 raw SVG → sprite 를 조립하는 스크립트가 없다(외부에서 한 번 조립해 커밋한 정적
파일). 그래서 "sprite 생성 스크립트 안에 치환을 넣는다"가 아니라 **sprite 파일을 직접
치환하는 별도 스크립트**로 만들었다. Figma Icon Set 에서 아이콘을 새로 받아 `icons/sprite.svg`를
다시 커밋할 때마다 이 스크립트를 다시 돌려야 한다 — 자동으로 안 걸린다(postbuild 에 안 걸음,
아래 "뒤집으려면" 참조 아님 — 의도적으로 수동 단계로 남겼다. sprite 재조립 자체가 이 저장소
밖에서 일어나는 수동 작업이라 그 뒤에 오는 이 단계도 같이 수동으로 뒀다).

`docs 동기화(sync-docs-assets.mjs)`는 손대지 않았다 — 이미 치환된 `icons/sprite.svg`를
그대로 복사하기만 하면 되므로.

### 근거

**정확히 `fill="#525252"` 문자열만 치환**해서 다른 색은 전혀 안 건드린다 — 전수 확인 결과
sprite 안의 fill 값은 293건이 전부 `#525252`이고 나머지 8건은 Social Button 브랜드 로고 심볼
(`icon-base-social-google`의 `#4285F4` 등 Google 4색·Naver `#03C75A`·Kakao `#FFEB3B`/`#3E2723`)
뿐이었다. 치환 후 재확인: 293건 모두 `currentColor`로 바뀌었고 브랜드 8건은 그대로, `<symbol>`
개수(1136)·`<svg>` 태그 균형 모두 변화 없음 — 구조가 안 깨졌다.

Headless Chrome 스크린샷으로 Alert·Icon Button·Social Button 세 페이지를 직접 확인했다.
Alert·Icon Button은 아이콘이 이제 색상별 토큰(fg)을 따라간다(이전엔 전부 회색). Social Button은
실제로는 `LOGO` 매핑이 `message-circle`/`smile`/`globe` 플레이스홀더만 쓰고 있어(스프라이트에
실제 브랜드 마크가 없다는 문서 설명과 별개로, `icon-base-social-*` 심볼 자체는 존재하지만
미사용) 애초에 브랜드색과 무관했다 — 오히려 컬러 배경 위에서 흰색/검정으로 올바르게 갈리는
것까지 확인(이전엔 배경과 무관하게 항상 회색이었을 결함).

`icon-duo-*`(838개, `stroke` 기반 duotone 아이콘군)는 이번 치환 대상이 아니다 — `fill` 이
아니라 `stroke="#171717"`/`stroke="#0EA5E9"`를 쓰고, 현재 어떤 컴포넌트·docs 페이지도
참조하지 않는 미사용 인벤토리라 범위 밖으로 남겨뒀다. 나중에 실제로 쓰기 시작하면 그때
같은 방식(stroke 치환)으로 판단한다.

### 뒤집으려면

`icons/sprite.svg`를 git 이력에서 치환 전 커밋으로 되돌린다. 또는 역방향 스크립트
(`fill="currentColor"` → `fill="#525252"`)를 만들어 돌린다. 건당 5분.

---

## ADR-023 · Figma↔SCSS 요소 대조를 하이브리드로 한다

상태: Accepted 확신도: 높음 날짜: 2026-09-01

### 맥락

28개 컴포넌트의 요소별 색 바인딩을 전수 대조할 방법이 필요했다. 완전 자동화가 가능한지
판단을 먼저 했다 — 3단계로 나눠보면:

1. **Figma 쪽 "요소→토큰" 추출**: `check-nodes.mjs`의 D1/D2가 이미 REST(`GET /v1/files/{key}/nodes`)
   + `tokens.ids.json`으로 이 일의 절반을 하고 있었다(노드 트리 순회 + `boundVariables` 이름
   해석, 인스턴스 내부는 제외). MCP `get_variable_defs`는 노드 트리 전체를 뭉뚱그려 집계해서
   요소별 구분이 안 된다(Alert에서 Icon·Close·Title이 전부 `alert-brand-fg` 하나로만 나와
   "아마 셋 다 이거겠다"는 정황 추론에 그쳤다) — REST+`tokens.ids.json`이 이 목적엔 더 정확했다.
2. **SCSS 쪽 파싱**: `check-tokens.mjs`가 이미 postcss로 컴파일된 CSS를 파싱한다(S5·S7a·S7b,
   ADR-020). 이것도 기존 인프라로 된다.
3. **"요소↔셀렉터" 매핑**: 이건 자동 도출이 안 됐다. Figma 레이어명(`Icon`)과 BEM 클래스명
   (`.alert__icon`)은 다른 명명 체계고, 이 매핑을 아는 건 사람(또는 Claude 앱)이 컴포넌트를
   조회하며 하는 바로 그 판단이다 — 지금 만드는 기준표 자체가 이 매핑과 동급의 작업물이다.

### 결정

**Figma 조회(REST)와 SCSS 파싱(postcss)은 자동화하고, 요소명↔셀렉터 매핑은 컴포넌트별 설정
파일로 사람이 정의한다.** 매핑이 없는 컴포넌트는 검사 대상에서 자연히 빠지고 지금처럼 프롬프트
기준표로 진행한다.

구현(Alert을 파일럿으로 완료):

- `scripts/check-nodes.mjs --dump <세트명>`: D1/D2의 `walk`/`resolveVarId`를 재사용해 그
  컴포넌트 세트의 모든 변형에 대해 `path → 바인딩된 토큰`을 위반 필터링 없이 JSON으로 낸다.
  element-map 작성·검증용.
- `scripts/lib/element-map/*.mjs`: 컴포넌트당 하나. `{ figmaPath, cssSelector, cssProperty,
  token, except?, via? }` 목록. `token`·`cssSelector`는 `{color}` 같은 변형 축 플레이스홀더를
  쓴다. `cssSelector`는 Figma 레이어가 아니라 **실제로 그 값이 선언되는 지점**을 가리킨다 —
  Icon·Close처럼 자체 선언이 없고 조상에서 상속받으면 조상 셀렉터를 가리키고 `via`에 이유를
  남긴다(자세한 근거는 `scripts/README.md` D14).
- **D14**(`check-nodes.mjs`에 통합, N군): 매핑마다 참조된 축만 전개해 대표 변형을 고르고,
  Figma 덤프·컴파일된 CSS(`dist/main.css`) 양쪽을 기대 토큰과 대조해 **Figma 불일치 / SCSS
  불일치를 구분해서** 보고한다.

### 근거

Figma 레이어명과 BEM 클래스명은 다른 명명 체계라 자동 도출이 불가능하다. 매핑 작성은 지금
기준표 작성과 조사량이 같아서(Alert처럼 단순한 컴포넌트는 조사를 구조화만 하면 되므로 추가
비용이 거의 없고, Table/Cell·Calendar·Modal·Avatar Group처럼 복잡한 컴포넌트는 `PROGRESS.md`
3~7단계에 반복 기록된 것처럼 애초에 한 번에 정확히 파악되지 않는 경우가 많아 매핑 작성 자체가
지금 하는 조사와 같은 작업량이다) 순증 비용이 없고, **한 번 쓰면 회귀 검사로 계속 재사용된다**
(ADR-014 "검사 항목은 누적한다"와 같은 방향).

Alert로 파일럿 구현 후 실제 REST 없이(FIGMA_TOKEN 만료 상태) `fetch`를 모킹한 통합 테스트로
로직을 검증했다 — 정상 매핑은 조용히 통과하고, Figma 쪽 데이터를 일부러 빠뜨리면 "Figma
불일치"로, `dist/main.css`를 일부러 깨뜨리면(임시 변경 후 재빌드로 원복) "SCSS 불일치"로 정확히
구분되는 것을 확인했다.

### 한계

**토큰 참조 오류만 잡는다.** 여러 클래스가 동시에 걸렸을 때 소스 순서·specificity로 실제
적용되는 값이 뭔지는 시뮬레이션하지 않는다 — Alert의 banner 테두리 버그(`.alert-banner`의
`border: none`이 나중에 나오는 `.alert-brand`의 `border`에 소스 순서상 덮어써진 것)가 그
예다. 두 선언 다 올바른 토큰을 참조하고 있어서 D14로는 안 잡히고, 실제로 오늘 이 버그는
사람(Claude 앱의 기준표 지시)이 스크린샷을 보고서야 잡혔다. 색 토큰 바인딩만 다루고, 효과
(그림자)·타이포·간격은 대상이 아니다.

### 뒤집으려면

`scripts/lib/element-map/` 디렉터리를 삭제하고 `check-nodes.mjs`의 D14 블록(`--dump` 모드
포함)을 제거한다. 프롬프트 기준표 방식으로 돌아간다. 30분.

---

## ADR-024 · docs 사이트에 다크모드 토글을 추가한다

상태: Accepted 확신도: 높음 날짜: 2026-09-01

### 맥락

`docs/` 의 모든 페이지가 `<html data-theme="light">` 로 고정돼 있어, `[data-theme='dark']`
오버라이드가 있는 토큰·컴포넌트(체크박스·드롭다운·칩·알럿 등 다수)를 이 문서 사이트에서
육안으로 검증할 방법이 없었다. 컴포넌트 CSS(`main.css`) 자체는 이미 다크 오버라이드를
전부 갖고 있어 이론상 `data-theme` 속성만 바꾸면 즉시 반영된다.

다만 실제로 구현해보니 컴포넌트 CSS와 별개로 **docs 사이트 자체의 크롬(`docs.css`)** —
사이드바·카드·페이지 헤더 배경/보더/텍스트 색 — 이 시맨틱 토큰이 아니라 `#fff`·`var(--neutral-*)`
같은 라이트 전용 리터럴로 하드코딩돼 있어서, 토글만 달면 컴포넌트 데모는 다크로 바뀌는데
그 주변 문서 UI는 계속 흰 배경으로 남는 반쪽짜리 다크모드가 될 뻔했다.

### 결정

`docs/assets/nav.js`에 다크모드 토글 버튼(`.icon-btn` 컴포넌트 재사용)을 사이드바 헤더에
추가하고, `localStorage`(`bluesoft-docs-theme`)로 선택을 저장해 페이지 이동 간 유지한다.
동시에 `docs/assets/docs.css`의 하드코딩된 배경/보더/텍스트 색을 `--surface-*`·`--border-*`·
`--text-*` 시맨틱 토큰으로 교체해 문서 크롬도 함께 전환되게 한다.

### 근거

컴포넌트 쪽 CSS는 이미 완성돼 있어 토글 자체의 구현 비용은 버튼 하나 + `localStorage` 읽고
쓰기 정도로 낮다. 컴포넌트별로 별도 다크 섹션을 만드는 대안은 26개 페이지 전체에 중복 콘텐츠를
만드는 것이라 비용이 훨씬 크고, 검사 스크립트만으로 대체하는 대안은 이 문서 사이트를 만든
목적(시각적으로 볼 수 있는 문서) 중 다크모드 부분을 포기하는 것이다.

### 뒤집으려면

`nav.js`의 `THEME_KEY`·`storedTheme` 블록과 `#doc-theme-toggle` 관련 코드를 제거하고,
`docs.css`의 시맨틱 토큰을 원래 리터럴로 되돌린다. 15분.

---

## ADR-025 · 미사용 space()/radius() 유효성 검증 함수를 제거한다

상태: Accepted 확신도: 높음 날짜: 2026-09-01
2026-09-08 확인: 결정한 세 가지가 모두 반영돼 있어 Accepted 로 바꿨다.

### 맥락

이 항목은 이 세션이 시작되기 전부터 워킹트리에 커밋 안 된 상태로 이미 존재하던 변경이다
(`scss/abstracts/_functions.scss` 삭제 + `main.scss`·`_maps.scss`·`CLAUDE.md` 연동 수정).
누가·왜 했는지 세션 안에서는 확인할 수 없어, 이 변경이 안전한지를 직접 검증했다.

`_functions.scss`는 커밋 f7bccc7("abstracts 계층 신설")에서 `space($key)`/`radius($key)`
함수로 도입됐다 — `$space-*`/`$radius-*` alias 위에 "존재하지 않는 키를 컴파일 에러로
드러내는" 유효성 검증만 얹는 래퍼였다. `scss/` 전체를 `space(`·`radius(` 로 grep했지만
실제 호출부가 **0건**이었다 — 도입된 뒤로 한 번도 실사용되지 않은 죽은 코드였다.

### 결정

`_functions.scss` 삭제, `main.scss`의 `@use 'abstracts/functions';` 제거, `CLAUDE.md`
파일 구조표에서 해당 행 제거를 그대로 받아들인다.

### 근거

호출부가 0건이라 실질적 효과가 없었고, 원래 목적(오탈자 방지)은 함수 없이도 이미 달성된다 —
`$space-4` 같은 존재하지 않는 SCSS 변수를 참조하면 Sass가 자체적으로 컴파일 에러를 낸다.
유지 비용(파일 하나, `main.scss` 등록, 문서화)만 있고 이득이 없는 죽은 추상화였다.

### 뒤집으려면

f7bccc7의 `scss/abstracts/_functions.scss` 내용을 복원하고, `main.scss`에
`@use 'abstracts/functions';`를 다시 추가하고, `CLAUDE.md` 파일 구조표에 해당 행을
다시 추가한다. 5분.

---

## ADR-026 · Wide 모드는 유지한다. 코드에서 최상단 분기만 만들지 않는다

상태: Accepted 확신도: 높음 날짜: 2026-09-07
초안에서 뒤집힌 결정이다 — 아래 "철회한 결정" 참고.

### 맥락

Breakpoint 컬렉션은 Wide · Desktop · Laptop · Tablet · Mobile 5모드다.
값을 대조하면 **Wide 와 Desktop 은 34개 중 2개만 다르다** — `grid/margin`(360 vs 120)과
`breakpoint`(1920 vs 1440). 나머지 32개는 같다. (Tablet 과 Laptop 은 23개가 달라 둘 다 필요하다.)

값만 보면 Wide 는 "값이 갈리지 않는 모드" 로 보였다. 그래서 초안에서는 삭제를 결정했다.
**전수 조사에서 뒤집혔다.**

### 전수 조사 결과 (2026-09-07)

페이지 단위로 전 노드의 `boundVariables` 를 훑었다.

| 페이지 | 노드 수 | `breakpoint` | `grid/*` |
|---|---|---|---|
| `❖ Prototype` | 13,204 | **약 50건** | `grid/gutter` 5곳 |
| `❖ Component` | 1,747 | 4건 | `grid/margin` 2곳 |
| `❖ Drafts` | 11,410 | 0 | 0 |
| `❖ Planning` | 2 | 0 | 0 |

`breakpoint` 는 **시안 아트보드의 `width` 에 바인딩돼 있다.**
랜딩 3종(TypeA · TypeB · Confirmed)이 1920 · 1024 · 375 로 각각, 진료예약안내 서브페이지 6개,
병원소개 5개, 클리닉 상세 2개, `hero` · `doctors` · `notice-bar` · `mobile-header` 인스턴스까지.

**사고가 아니라 확립된 설계 패턴이다.** 아트보드에 모드를 지정하면 폭이 따라오게 만든 것이고
실제로 동작하고 있다.

`grid/*` 도 쓰이고 있다 — `grid/gutter` 는 GRID 프레임의 행·열 간격
(`doctors` · `facility-floors__rooms` · `column-grid` · `clinic-list__grid` · `img-box`),
`grid/margin` 은 `mobile-nav__header` · `mobile-nav__footer` 의 좌우 패딩.

### 철회한 결정

**"Wide 모드를 삭제한다" 를 철회한다.**

철회 근거는 하나다. 초안은 `breakpoint` 가 **"노드에 바인딩되지 않는 참조값"** 이라는 전제 위에
서 있었는데, 그 전제가 사실이 아니었다. `breakpoint` 는 아트보드 폭으로 쓰이고 있고
Wide(1920)와 Desktop(1440)의 차이가 실제로 일하고 있다.
따라서 `FIGMA.md` 「컬렉션과 모드」의 "모드는 실제로 값이 갈릴 때만 만든다" 에 **위배되지 않는다.**

지웠다면 1920 아트보드 20여 개가 한꺼번에 1440 으로 줄었다.

### 결정

1. **Wide 모드를 유지한다.** Breakpoint 는 5모드 그대로 간다
2. **`_breakpoint.scss` 에 최상단(1920) 미디어쿼리를 만들지 않는다.**
   Desktop 위로는 다른 값이 같아 분기가 불필요하고, `min-width: 1920px` 은 브라우저마다 갈린다
3. **풀블리드는 모드를 늘리지 않고 `padding-inline: clamp(…)` 로 처리한다**
4. **`breakpoint` 토큰의 description 을 실제에 맞게 고쳤다.**
   기존 "노드에 바인딩하지 않는다" 는 사실과 달랐다
5. **`grid/margin` 은 유지한다.** `mobile-nav` 에서 쓰이고 있다

### 부수 변경

`site-header` `Layout=center, Theme=light`(`110:4679`)와 `site-footer` `Mode=dark`(`295:6408`)의
`width` 에서 `breakpoint` 바인딩을 풀었다. 둘 다 `layoutSizingHorizontal = FILL` 이라
부모 폭을 따라가는 노드에 폭 변수가 같이 물려 있는 모순 상태였다.

**FILL 로 바꿔도 폭 바인딩은 자동으로 풀리지 않는다.** 이것이 "FILL 로 고쳤는데 계속 잡힌다" 의
원인이었다. `setBoundVariable('width', null)` 로 따로 풀어야 한다.

### 근거

값 개수만으로 모드의 필요성을 판정할 수 없다. 34개 중 2개만 달라도 **그 2개가 일하고 있으면
모드는 일하는 것이다.** 판정은 "값이 다른가" 가 아니라 "다른 값이 어디에 쓰이는가" 로 한다.

### 뒤집으려면

Wide 삭제로 돌아가려면 먼저 1920 아트보드 20여 개의 `width` 바인딩을 풀고 고정 폭으로 바꿔야 한다.
아트보드 폭이 모드를 따라오는 이점을 잃으므로 권하지 않는다.

---

## ADR-027 · 입력 계열의 radius 를 버튼과 분리한다

상태: Accepted 확신도: 높음 날짜: 2026-09-07

### 맥락

입력 상자·텍스트영역·셀렉트의 모서리 반경이 버튼용 토큰을 그대로 쓰고 있었다.
1장 "형제 참조를 만들지 않는다" 에 걸리는 상태다. 버튼 모서리를 바꾸면 입력 상자도 같이 바뀐다.

ButtonGroup 설계에서 이 결합이 문제가 됐다. 그룹은 붙는 쪽 모서리만 각지게 만드는데,
입력과 버튼이 같은 토큰을 보고 있으면 한쪽을 조정할 때 다른 쪽이 끌려온다.
그룹을 먼저 만들면 결합이 더 굳어진다.

### 결정

Shape 컬렉션에 입력 계열 전용 radius 토큰을 새로 만들고, 입력 상자 계열 마스터를
그쪽으로 옮겼다. 모드별 참조 대상은 버튼용 토큰과 동일한 primitive 로 맞췄다 —
**지금은 값이 같고, 앞으로 따로 움직일 수 있는 상태**가 목표였다.

- 새 토큰의 scopes 는 모서리 반경 하나로 조였다
- codeSyntax 를 등록해 코드에서 참조할 이름을 고정했다
- 옮긴 대상은 입력·텍스트영역·셀렉트 마스터의 모서리 바인딩 전부

### 검증

옮긴 뒤 다시 조회해 확인한 것 두 가지.

| 확인 | 결과 |
|---|---|
| 입력 계열에 버튼 토큰이 남았는가 | 0건 |
| 버튼 쪽 사용처가 줄었는가 | 그대로 |

### 근거

값이 같을 때 분리하는 것이 가장 싸다. 값이 달라진 뒤에 분리하면 어느 쪽이 원래 의도였는지
구분할 수 없고, 그 사이에 만들어진 컴포넌트가 잘못된 쪽을 참조하게 된다.

### 뒤집으려면

입력 계열 마스터의 모서리 바인딩을 버튼용 토큰으로 되돌리고 새 토큰을 삭제한다.
컴포넌트 수가 적어 되돌리기 부담은 크지 않다. 다만 되돌리면 형제 참조 상태로 돌아간다.

### 남은 일

SCSS 쪽은 아직 버튼용 CSS 변수를 쓰고 있다. 아래 미결에 올렸다.

---

## ADR-028 · ButtonGroup 과 Field/Group 을 따로 만든다

상태: Accepted 확신도: 높음 날짜: 2026-09-07

### 맥락

요청은 "input + 버튼" 이었고 참고 기준은 Bootstrap 5 이상이었다.
Bootstrap 은 이 둘을 별개로 둔다 — 버튼끼리 붙이는 것과, 입력 앞뒤에 무언가를 붙이는 것.
용도가 다르고 붙는 재료가 다르다.

기존 컴포넌트 중에 붙은 버튼의 선례는 있었지만(모달 액션·세그먼티드·페이지네이션)
셋 중 어느 것도 "입력 + 버튼" 을 하지 않는다. 신규가 맞다고 판단했다.

### 결정

**두 컴포넌트로 나눈다.**

| 컴포넌트 | 하는 일 |
|---|---|
| 버튼 그룹 | 버튼을 좌우로 붙여 나열한다 |
| 필드 그룹 | 입력 상자 앞뒤에 애드온을 붙인다 |

아래 다섯 가지가 이 설계의 실질이다.

#### 1. 크기는 그룹에서 한 번에 정한다

Bootstrap 이 컨테이너에서 크기를 정하는 이유를 그대로 따랐다.
항목별로 크기를 주면 같은 그룹 안에서 높이가 어긋난다.
**크기는 변형 축으로 두고, 항목에는 크기를 따로 주지 않는다.**

처음에는 크기 축을 "중복" 으로 보고 빼려 했다. 잘못이었다 —
그룹 차원의 크기가 없으면 같은 그룹 안 버튼들의 높이를 보장할 수 없다.

#### 2. 안쪽 모서리는 변형으로 결정한다

Bootstrap 은 CSS 선택자로 첫 항목·마지막 항목의 모서리를 각지게 만든다.
**Figma 에는 그런 선택자가 없다.** 슬롯에 버튼을 그대로 넣으면 모서리를 매번 손으로 고쳐야 하고,
그 순간 인스턴스 오버라이드가 되어 마스터 변경을 못 받는다.

Figma 는 모서리 네 개를 각각 따로 바인딩할 수 있다. 이 점을 이용해
첫 항목은 왼쪽만 둥글게, 중간은 전부 각지게, 마지막은 오른쪽만 둥글게 미리 만들어 두었다.

#### 3. 별도의 항목 래퍼 컴포넌트를 만들지 않았다

당초 안은 `first · middle · last` 를 변형 축으로 갖는 내부용 항목 컴포넌트를 두는 것이었다.
만들지 않았다. 항목 수를 변형 축으로 두면 각 변형 안에서 모서리 패턴이 이미 확정되므로,
래퍼가 하려던 일이 그룹 안에서 끝난다. 컴포넌트 하나를 줄였다.

#### 4. 애드온 유무는 BOOLEAN 이 아니라 변형 축이다

공통 방침은 "콘텐츠는 BOOLEAN 토글 + 인스턴스 교체로 담는다" 다. 여기서는 따르지 못했다.

**BOOLEAN 토글로는 모서리 반경을 바꿀 수 없다.** 앞쪽 애드온이 켜지면 입력 상자의 왼쪽 모서리가
각져야 하는데, 토글은 표시 여부만 바꾸고 바인딩은 그대로 둔다.
애드온 위치를 변형 축으로 올려야 모서리가 변형과 함께 따라온다.

변형 수가 늘어나는 대가를 치렀다. 대신 앞뒤에 무엇을 붙일지는 인스턴스 교체로 남겨
버튼과 아이콘 버튼 둘 다 받을 수 있게 했다.

#### 5. 테두리 있는 버튼은 붙는 조합에서 제외한다

테두리가 있는 버튼을 나란히 붙이면 맞닿은 곳에 1px 이 두 겹으로 보인다.
Bootstrap 은 음수 마진으로 겹쳐서 처리하는데 **Figma 오토레이아웃은 음수 간격을 못 준다.**

이 시스템의 버튼은 테두리 계열 하나를 빼면 테두리가 없다. 그래서
**항목 사이에 1px 구분선 도형을 따로 두는 방식**을 골랐고, 그 색을 위한 토큰을
Theme 컬렉션에 새로 만들어 기존 테두리 색을 참조하게 했다.
테두리 계열 버튼은 이 조합에서 쓰지 않는다.

### 그 밖에

- **가장 작은 크기는 그룹에 넣지 않았다.** 버튼은 그 크기가 있지만 입력에는 없다.
  붙일 상대가 없는 크기를 그룹에 두지 않는다
- **라벨과 힌트는 그룹 밖에 둔다** (Bootstrap 규칙). 기존 입력 컴포넌트의 표시 토글이
  상태·검증 변형을 바꿔도 유지되는지 먼저 확인했고, 유지된다 — 입력 마스터를 쪼갤 필요가 없었다
- 컴포넌트 설명에 접근성 요구를 남겼다 — 그룹에 역할과 이름, 아이콘만 있는 버튼에는 대체 텍스트

### 근거

Bootstrap 이 둘을 나눠 놓은 이유가 이 시스템에도 그대로 적용된다.
하나로 합치면 "무엇이 들어올지 모르는 슬롯" 이 되고, 그 순간 높이를 맞춘다는 보장이 사라진다.

### 뒤집으려면

두 컴포넌트를 지우고 슬롯 하나짜리 컨테이너로 바꾼다. 붙인 화면이 있으면 같이 손대야 한다.
크기 보장과 안쪽 모서리 문제가 되살아나므로 권하지 않는다.

### 남은 일

필드 그룹의 크기 변형 나열 순서가 어긋나 있다. 자식 순서를 바꿔도 변형 목록 순서는
따라오지 않아 Figma UI 에서 직접 고쳐야 한다.

애드온 교체 방식은 ADR-029 에서 바뀌었다. 교체 속성을 두지 않는다.

---

## ADR-029 · 애드온 종류를 교체 속성이 아니라 변형 축으로 둔다

상태: Accepted 확신도: 높음 날짜: 2026-09-07
ADR-028 의 애드온 교체 방식을 바꾼다. 나머지 결정은 그대로다.

### 맥락

Field/Group 의 애드온 버튼 크기가 크기 변형을 따라가지 않았다.
입력 상자는 크기별로 맞게 나오는데 애드온만 한 크기에 고정돼 있었다.

원인을 두 번 잘못 짚었다. 세 번째에 통제 사례로 확정했다.

**인스턴스 교체 속성은 변형 집합 전체에 대해 값이 하나다.** 변형별로 다른 값을 가질 수 없다.
그래서 어떤 레이어의 원본 컴포넌트를 그 속성이 결정하고 있으면, 그 레이어는 모든 변형에서
같은 것을 가리킨다. 크기 축을 나눠도 마찬가지다.

같은 집합 안에 통제 사례가 있었다.

| 레이어 | 교체 속성에 묶였는가 | 변형별 값 |
|---|---|---|
| 앞쪽 애드온 (전 변형) | 예 | 전부 동일 |
| 뒤쪽 애드온 (한 변형 제외) | 예 | 전부 동일 |
| 뒤쪽 애드온 (그 한 변형) | 아니오 | **독립된 값 유지** |

앞쪽과 뒤쪽이 서로 독립인 것도 서로 다른 교체 속성에 묶여 있어서였다.

### 철회한 진단

기록으로 남긴다. 둘 다 틀렸다.

| 틀린 진단 | 왜 틀렸나 |
|---|---|
| 플러그인 API 의 한계다 | Figma UI 에서 손으로 바꿔도 같이 전파된다. 사용자가 직접 확인했다 |
| 변형을 집합 안에서 복제해 만들어 자식을 공유한다 | 제작 순서와 무관하다. 교체 속성 유무로만 갈린다 |

레이어 이름을 임시로 바꿔 매칭을 끊는 방법, `swapComponent` 로 경로를 바꾸는 방법 둘 다
전파를 막지 못했다. 속성에 묶인 값을 바꾸는 것이라 경로를 바꿔도 결과가 같다.

### 결정

**교체 속성 두 개를 삭제하고, 애드온 종류를 변형 축으로 올린다.**

교체 가능성과 변형별 차이는 같은 레이어에서 동시에 가질 수 없다.
**변형은 이 제약이 없다 — 변형마다 다른 값을 가질 수 있다.** 그래서 종류를 변형 축으로 옮겼다.

축이 하나 늘어 변형 수가 두 배가 된다. 그 대신 셋 다 얻는다.

| | 교체 속성 | 변형 축 |
|---|---|---|
| 크기가 변형을 따라가는가 | 아니오 | 예 |
| 목록에서 고를 수 있는가 | 예 | 예 |
| 크기를 바꿔도 종류가 유지되는가 | 아니오 | 예 |

수동 인스턴스 교체로 두는 방법도 검토했다. 크기는 지킬 수 있지만
**Figma 문서상 컴포넌트를 통째로 교체할 때 텍스트 오버라이드만 보존되고**,
중첩 인스턴스 교체가 변형 전환에서 풀린다는 사례가 보고되어 있다.
크기를 바꾸면 종류가 되돌아갈 수 있어 택하지 않았다.

앞뒤에 서로 다른 종류를 섞는 조합은 만들지 않았다. 만들면 조합이 더 늘어나고,
지금 그 조합을 쓸 화면이 없다. 필요해지면 그때 추가한다.

### 부수 변경

- 애드온 레이어 전부를 교체 속성 연결에서 풀고, 속성 두 개를 삭제했다.
  남은 속성은 크기와 애드온 위치 두 축뿐이다
- 애드온 크기를 각 변형에 맞게 지정했다. 속성을 뺀 뒤에는 전파되지 않는다
- 종류 축을 추가하고 아이콘 버튼 변형 12개를 만들었다. 변형은 12개에서 24개가 됐다
- 축을 추가하면서 크기 나열 순서가 저절로 바로잡혔다. 남은 것은 애드온 위치 축 하나다
- 레이어 이름이 하나만 달랐던 것을 통일했다. 변형을 전환할 때 Figma 는 레이어 이름으로
  오버라이드를 매칭하므로, 이름이 다르면 그 변형에서 오버라이드가 끊긴다
- 컴포넌트 설명에 교체 방법과 "교체한 뒤 크기를 맞춘다" 를 남겼다

### 검증

전 변형을 별도 호출로 다시 조회했다. 불일치 0건.

| 확인 | 결과 |
|---|---|
| 애드온 크기가 변형 크기와 같은가 | 12개 변형 전부 일치 |
| 애드온·입력 상자·묶음 높이가 같은가 | 전부 일치 |
| 묶음이 세로로 내용에 맞춰지는가 | 유지 |
| 맞닿는 모서리 처리가 살아있는가 | 애드온·입력 상자 양쪽 유지 |
| 애드온 종류가 축과 맞는가 | 24개 변형 전부 일치 |
| 남은 속성 | 크기·애드온 위치·종류 세 축. 교체 속성 없음 |

**쓰기와 같은 스크립트에서 읽은 높이는 신뢰할 수 없다.** 레이아웃 재계산 전 값이 나온다.
이 때문에 프레임이 고정 높이로 망가진 것처럼 보인 적이 있다. 정상이었다.
검증은 반드시 별도 호출로 한다.

### 근거

크기 보장이 없으면 같은 묶음 안에서 높이가 어긋난다. 그것을 막는 것이 이 컴포넌트의 일이다.
교체 목록은 편의이고, 없어도 같은 일을 할 수 있다.

### 뒤집으려면

종류 축의 아이콘 쪽 변형 12개를 지우고 축을 없앤다. 그다음 애드온 레이어에 교체 속성을
다시 만들어 연결한다. 그 순간 모든 변형의 애드온이 한 크기로 고정되므로,
크기 축도 함께 없애야 앞뒤가 맞는다.

### 일반 규칙

**변형마다 달라야 하는 것은 변형 축에 둔다. 교체 속성에 두지 않는다.**

교체 속성은 집합 전체에 값이 하나여서 변형별 차이를 담지 못한다.
둘 다 필요하면 축을 하나 더 만든다 — 변형 수가 늘어나는 것이 그 대가다.
설계 단계에서 정해야 한다. 나중에 고칠 수 있는 문제가 아니다.

---

## ADR-030 · `$css-breakpoint-lg` 를 1280 에서 1440 으로 맞춘다

상태: Accepted 확신도: 높음 날짜: 2026-08-31
7단계 작업 중에 정해졌으나 진행 기록 안에만 적혀 있었다. 2026-09-08 문서 정리에서 ADR 로 옮겼다.

### 맥락

컴파일타임 상수 `$css-breakpoint-lg` 가 1280 이었고, `tokens/_breakpoint.scss` 의
미디어쿼리는 Figma `Breakpoint` 컬렉션의 기준폭을 그대로 써서 1440 이었다.
"헤더 nav 전환은 1280 에서, 디자인 토큰 값 전환은 1440 에서" 라는 어긋남이 생겼다.

### 근거

Header 컴포넌트의 `Width` 축에는 모바일 폭과 데스크톱 폭 두 종류만 있고
**태블릿 구간을 보여주는 변형이 없다.** `--header-height` 도 정확히 Figma 데스크톱
기준폭에서 값이 바뀐다. 즉 Figma 가 실제로 보여주는 데스크톱 진입점은 1440 이고,
**1280 은 근거가 된 적이 없다.**

변경의 안전성도 확인했다. 그 상수를 소비하는 `mq()` 믹스인이 코드베이스 어디에서도
호출되지 않고(grep 확인), `_header.scss` 자신의 메뉴 전환은 모바일 기준폭만 쓴다.
그래서 **이 변경은 당시 컴파일 결과를 바꾸지 않았다.** 나중에 `mq('lg')` 를 쓰기
시작할 때를 위해 미리 Figma 와 맞춰둔 것이다.

### 결정

`$css-breakpoint-lg` 를 Figma 데스크톱 기준폭에 맞춘다.
근거는 `tokens/_breakpoint.scss` 에 주석으로 남긴다.

### 남은 일

**태블릿 구간에서 헤더 메뉴가 어떻게 보여야 하는지는 여전히 근거가 없다.**
Figma 에 그 구간 변형 자체가 없다. 현재 코드는 모바일 기준폭 이상에서 이미 풀 메뉴를
보여주는데, 이걸 데스크톱 기준폭까지 미루는 게 맞는지는 별도 판단이 필요하다.
`STATUS.md` 「막힌 것」에 올려뒀다.

---

## ADR-031 · 문서 경계를 주제가 아니라 읽는 사람으로 자른다

상태: Accepted 확신도: 높음 날짜: 2026-09-08

### 맥락

`CLAUDE.md` 하나에 Figma 규칙과 SCSS 절차가 같이 있었다. 문서가 커지자 내용을
`figma.md` 로 넘겼는데, 그 파일은 Cowork 가 읽을 수 없는 위치에 있었고 내용은 이미
스킬로 옮겨진 상태였다. 참조는 장 번호로 걸려 있었다.

**결과: 끊긴 참조가 저장소 전체에 여러 곳 생겼다.** `CLAUDE.md` 에서 다섯 곳,
`scripts/README.md` 에서 스물세 곳. 규칙이 필요한 대목마다 없는 문서를 가리켰다.
그중 하나는 가리키는 장 번호 자체가 이미 다른 내용이었다.

같은 시기에 진행 기록은 끝난 일의 상세 메모가 쌓여 원래 목적(세션 재개)에
쓸 수 없을 만큼 길어졌다. 그 안에 결정 하나(ADR-030)가 묻혀 있었다.

### 결정

**주제로 자르지 않고 읽는 사람으로 자른다.** 한 문서를 읽는 동안 다른 문서로
넘어가지 않아도 작업이 끝나야 한다.

| 문서 | 읽는 사람 |
| --- | --- |
| `CLAUDE.md` | 전원. 누가 무엇을 하고 어디를 읽는지만. 규칙을 두지 않는다 |
| `FIGMA.md` | Figma 를 고치는 사람 |
| `SCSS.md` | SCSS 를 쓰는 사람 |
| `STATUS.md` | 전원. 지금 어디까지 왔나 |

찾을 때만 여는 문서는 세 개로 한정한다 — `DECISIONS.md`(왜), `scripts/README.md`(검사 항목),
`figma/tokens.*.json`(값). 이 셋만 참조 대상으로 허용한다.

프로젝트와 무관한 Figma 공통 지식은 저장소에 두지 않고 `figma-work-principles` 스킬에 둔다.
커서와 Cowork 양쪽이 읽을 수 있는 곳은 계정 스킬과 저장소뿐이므로,
`~/.claude/common/` 은 공통 문서 위치로 쓰지 않는다.

### 함께 정한 규칙

1. **참조는 제목으로 한다. 장 번호로 하지 않는다** — 번호는 문서가 자라면 밀린다.
   이 사고의 직접 원인이었다. `CLAUDE.md` 의 장 번호를 없애 구조적으로 막았다
2. **넘기고 싶어지면 경계를 잘못 자른 것이다** — 옮기거나 합친다. 사본은 만들지 않는다
3. **값·수치·개수를 적지 않는다** — 검사 항목 S8 이 막는다
4. **끝난 것은 지운다** — git 이 기억한다

### 함께 발견한 것

**S8 은 `scripts/README.md` 에 정의만 있고 구현이 없었다.** 그래서 3번 규칙이
한 번도 강제된 적이 없고, 진행 기록의 실측 개수가 낡은 채로 남아 있었다.
이번에 구현했다.

### 폐기

`PROGRESS.md` · 루트 `README.md` · `~/.claude/common/figma.md` · Cowork 프로젝트 `figma.md`.
내용은 git 과 스킬에 있다.

`scripts/README.md` 는 남긴다. **검사 항목은 Figma 쪽이 설계하고 코드 쪽이 실행하므로
두 사람이 다 본다.** 한쪽 문서에 넣으면 다른 쪽이 넘어와야 한다 — 그래서 참조 문서로 둔다.

---

## ADR-032 · (철회) 배포 방식

상태: Superseded 날짜: 2026-09-11

**배포는 개발자가 담당한다.** 이 저장소가 배포 방식을 정하지 않는다.
레지스트리·CDN·버전 정책을 정했던 내용은 지웠다 — 우리가 지킬 규칙이 아니게 됐다.

**다만 아래는 배포 주체와 무관하게 남는다.**

- **클래스 이름과 토큰 이름은 배포되는 순간 공개 API 가 된다.** 바꾸면 쓰는 쪽을 전부 고쳐야 한다
- **아이콘 sprite 와 로고도 배포물이다**

---

## ADR-033 · 이름에 `bds` 접두사를 붙이고 첫 배포 전에 한 번에 바꾼다

상태: Accepted 확신도: 높음 날짜: 2026-09-09

### 맥락

접두사 없이 쓰는 컴포넌트 클래스 아홉 개가 Bootstrap 과 이름이 완전히 같다 —
`.btn` `.btn-link` `.alert` `.card` `.dropdown` `.modal` `.table` `.tooltip`,
그리고 chip 을 badge 로 바꾸면 `.badge` 까지. 한 페이지에서 둘 다 로드되면
나중에 로드된 쪽이 이긴다.

접두사 후보로 먼저 검토한 두 가지는 각각 이유가 있어 탈락했다.

- **`bs`** — Bootstrap 의 기본 CSS 변수 접두사다. 충돌을 피하려고 붙이는 접두사인데
  `--bs-badge-color` · `--bs-btn-color` 가 Bootstrap 것과 정확히 같아진다
- **`blue`** — 프리미티브 색 램프(`--blue-50` ~ `--blue-950`)와 같은 자리에 섞인다

### 결정

| 대상 | 이름 |
| --- | --- |
| CSS 변수 | `--bds-*` |
| 컴포넌트 클래스 | `.bds-*` |
| Figma 토큰그룹 | `button/*` → `btn/*` |
| 컴포넌트 | `chip` → `badge` |
| `text` · `border` | 줄이지 않는다 |

**접두사는 Figma 변수 이름이 아니라 코드 쪽 이름이다.** Figma 는 `text/primary` 로 두고
코드로 옮길 때 `--bds-text-primary` 가 된다. `codeSyntax` 만 그에 맞춰 채운다.

### 근거

- `bds` 는 패키지 이름 `blue-ds` 의 약자다. 세 글자고 색 이름과 겹치지 않으며,
  잘 알려진 프레임워크 중 쓰는 곳을 찾지 못했다
- 시스템 약자를 앞에 붙이는 것은 토큰 네이밍의 표준 관례다 — `slds-` `mds-` `cds-`
- 배포되는 디자인시스템은 클래스에도 접두사를 붙인다 — `.slds-button` `.cds--btn` `.spectrum-Button`
- **`text` → `tx`, `border` → `bo` 는 하지 않는다.** 쓰는 시스템을 찾지 못했고,
  `tx` 는 transaction 으로 더 많이 읽힌다. 토큰 이름은 쓰는 횟수보다 읽는 횟수가 훨씬 많다
- **`button` → `btn` 만 예외인 이유는 축약이 아니라 정리이기 때문이다.**
  클래스는 이미 `.btn` 인데 변수만 `--button-*` 이라 둘이 어긋나 있었다

### 왜 지금인가

**첫 배포 전에만 가능하다.** 배포 후에는 클래스와 토큰 이름이 공개 API 이고,
바꾸면 메이저 버전이며 설치한 프로젝트를 전부 고쳐야 한다.

### 한 번에 바꾼다

접두사·`btn`·`badge` 는 같은 이름을 건드린다. 나눠서 하면 1,000건짜리 기계적 변경과
검증을 두 번 하게 된다. **한 커밋으로 처리한다** — 중간 상태는 문서 사이트가 스타일 없이
뜨는 깨진 상태다.

손으로 고치지 않는다. 매핑을 스냅샷과 SCSS 선택자에서 자동으로 뽑고 스크립트가 한 번에
바꾼다. 이미 `--bds-` 로 시작하면 건너뛰게 해서 두 번 돌려도 안전하게 만든다.

**검증은 컴파일 결과로 한다.** 리네임 전 `dist/main.css` 를 보관해 두고, 리네임 후
다시 빌드해서 양쪽에서 접두사만 벗겨 비교한다. 완전히 같아야 한다 — 다르면 리네임이
이름 말고 다른 것도 건드린 것이다.

### 뒤집으려면

배포 전이면 같은 스크립트를 역방향으로 돌린다. 배포 후에는 메이저 버전과
별칭 유예 기간이 필요하다 — 배포를 맡은 쪽과 절차를 맞춘다.

---

## ADR-034 · Alert 의 모서리 반경을 Shape 컬렉션 토큰으로 분리한다

상태: Accepted 확신도: 높음 날짜: 2026-09-09

### 맥락

Alert 의 inline·toast 변형이 `radius/md` 를 **직접** 바인딩하고 있었다.
button·card·tabs 등은 Shape 컬렉션에 자기 토큰이 있어 각진 모드에서 0 이 되는데,
Alert 은 없어서 **각진 모드에서 혼자 둥글게 남았다.**

같이 발견된 것이 있다. 코드는 `$radius-lg`(16px)를 쓰고 있었는데 Figma 는 12px 이었다.
**8단계 전수 대조의 범위가 "색 바인딩"이라 반경은 대조 대상이 아니었다.**
그래서 Alert 이 검사통과로 표시된 채로 4px 어긋나 있었다.

### 결정

`Shape` 컬렉션에 `alert/radius` 를 만들고 inline·toast 변형을 여기에 바인딩한다.

```
Rounded  radius/md
Square   radius/none
Pill     radius/md
```

Pill 에서도 Rounded 와 같은 값을 쓴다 — 알림 상자가 캡슐 모양이 되면 안 된다.
banner 변형은 항상 각져야 하므로 `radius/none` 직접 바인딩을 유지한다.

### 함께 정한 것

**8단계의 범위를 색에서 "바인딩된 값 전부"로 넓힌다.** 반경·간격·크기도 대조한다.
조회 한 번에 같이 나오는 정보라 추가 비용이 거의 없는데, 색만 보면 이번 같은 것을 놓친다.

---

## ADR-035 · 검사는 누적하지 않는다 — 도구가 덮으면 지운다

상태: Accepted 확신도: 높음 날짜: 2026-09-14

**ADR-014 를 뒤집는다.**

### 맥락

ADR-014 는 「발견한 문제 유형을 검사로 만들고, 항목은 줄지 않는다」고 정했다.
검사가 유일한 안전망이던 동안에는 맞았다.

그 뒤로 항목이 계속 늘었고, **검사를 만들고 고치고 검증하는 시간이 검사가 막아 준
시간보다 길어졌다.** 실제로 겪은 것 둘이다.

- 코드 안의 hex 를 보던 검사는 정규식이라 **주석 줄을 위반으로 잡았다.** 같은 일을 하는
  도구는 CSS 를 파싱해서 그런 오탐이 없다
- 코드와 스냅샷을 대조하던 검사는 **토큰 파일이 사람이 옮겨 적은 사본이었기 때문에**
  존재했다. 생성물로 바꾸니 어긋날 수가 없어져 검사할 것 자체가 없어졌다

### 결정

**검사는 누적하지 않는다.**

- **새 도구나 구조 변경이 어떤 검사를 덮으면, 그 검사를 같은 커밋에서 지운다.**
  안 지우면 둘 다 돌고, 겹치는 자리에서 **약한 쪽이 오탐을 낸다**
- **검사를 새로 만들기 전에, 이미 있는 도구가 덮는지 먼저 확인한다**
- **검사가 필요 없어지게 구조를 바꿀 수 있으면 그쪽을 택한다.** 사본을 지키는 검사보다
  사본을 안 만드는 구조가 낫다

### 근거

ADR-014 의 목표는 회귀 방지였고 그건 여전히 맞다. **바뀐 것은 회귀를 막는 수단이
우리 검사뿐이 아니라는 것이다.** 업계에 이미 있는 도구가 같은 일을 더 정확하게 한다.

검사를 지울 때는 **지우기 전에 대체 수단이 같은 것을 잡는지 확인한다.** 이번 두 건 모두
그렇게 했다 — 값이 하나도 안 바뀐 것을 보이고 나서 지웠다.

### 뒤집으려면

덮는 도구가 없는데 검사를 지웠다면 되살린다. 다만 그때는 **왜 그 도구로 부족했는지**를
같이 적는다.

---

## ADR-036 · stylelint 를 도입한다

상태: Accepted 확신도: 높음 날짜: 2026-09-14

**ADR-020 을 뒤집는다.**

### 맥락

ADR-020 은 중첩 깊이와 BEM 두 규칙을 강제하려고 stylelint 를 보다가, 전체 규칙을 돌리니
대부분이 이 프로젝트와 무관한 표기 취향이고 BEM 클래스가 오탐으로 잡혀서 **설정 유지 비용이
얻는 것보다 크다**고 판단했다. 그 판단은 그 목적에서는 옳았다.

### 무엇이 달라졌나

**이번에는 그 두 규칙을 강제하려고 본 게 아니다.** 실제로 돌려 보니 **우리 검사가 못 잡던
것**을 잡았다.

- **선언 순서 때문에 뒤 규칙이 앞 규칙을 덮는 자리**
- **같은 선택자를 두 번 쓴 자리**

둘 다 `SCSS.md` 「검사가 못 잡는 실수」에 **사람이 봐야 한다고 적어 두었던 항목**이다.
그리고 표기 잡음은 자동 수정으로 한 번에 정리된다 — ADR-020 이 비용으로 본 부분이
실제로는 명령 한 번이었다.

### 결정

**도입한다.** 설정은 저장소에 둔다.

**무엇을 덮나** — 캐스케이드 순서 · 중복 선택자 · 컴포넌트 코드 안의 hex.

**무엇을 안 덮나 — 이걸 반드시 같이 본다.**

- **중첩 깊이와 BEM 클래스 이름은 여전히 우리 검사가 본다.** 이번 설정은 그 둘을 안 건다
  (클래스 이름 규칙은 이 프로젝트 BEM 을 오탐해서 껐다). **stylelint 가 생겼다고 그 검사를
  지우면 안 된다**
- 프리미티브를 컴포넌트에서 직접 쓰는지는 우리 검사 몫이다
- 코드와 Figma 값 대조는 생성으로 없어졌다(ADR-035)

### 부딪힌 것과 처리

- **토큰 SCSS 는 생성물이라 대상에서 뺐다.** 포맷터가 hex 표기를 바꾸면 생성 결과가 흔들린다
- **hex 축약 규칙과 미디어쿼리 표기 규칙은 껐다.** 프로젝트가 정한 표기와 부딪힌다

### 뒤집으려면

설정 파일을 지우고 명령을 뺀다. 다만 그러면 **캐스케이드 순서와 중복 선택자를 다시 아무도
안 본다** — ADR-020 시점과 달리 지금은 그 둘이 실제로 잡히고 있다.


---

## ADR-037 · Tabs 는 Shape 모드를 따르지 않는다

상태: Accepted 확신도: 높음 날짜: 2026-09-14

### 맥락

`Shape` 컬렉션에 `tabs/radius` 가 있었는데 **Tabs 의 어떤 노드도 그것을 물지 않았다.**
pill·segmented 아이템과 Segment 컨테이너 전부 Primitive 의 완전 둥근 반경을 직접 바인딩한다.
코드도 라이브를 따라 그 값을 상수로 박았다(`SCSS.md` 「값의 출처」).

그래서 이 토큰은 **세 모드 값만 들고 아무 데서도 안 쓰이는 상태**였다.
다른 변수가 별칭으로 참조하지도 않았고, 생성된 토큰 SCSS 에만 이름이 남아 있었다.

### 결정

**Tabs 는 각진·Pill 모드를 지원하지 않는다.** 탭은 어느 모드에서도 완전 둥글다.
`tabs/radius` 를 `Shape` 에서 지운다. 노드 바인딩과 코드 상수는 그대로 둔다.

간격 토큰(`tabs/gap/*`)은 남긴다 — 세 모드 값이 같을 뿐 실제로 쓰이고 있다.

### 근거

`Shape` 컬렉션은 **모드에 따라 값이 달라져야 하는 것만** 담는다.
Tabs 가 모드를 안 따른다고 정한 이상 그 토큰은 정의상 자리가 없다.
남겨 두면 값은 있는데 아무도 안 쓰는 토큰이 되어 **반드시 낡고, 검사가 매번 불일치로 잡는다.**

### 뒤집으려면

`Shape` 에 토큰을 다시 만들고(Rounded 도 완전 둥근 값으로 둬야 지금 모양이 유지된다)
세 군데 노드에 바인딩한 뒤 스냅샷을 다시 뽑는다. 코드는 상수를 토큰 별칭으로 되돌린다.

---

## ADR-038 · 초점 표시는 테두리가 신호를 지고, 링은 장식이다

상태: Accepted 확신도: 높음 날짜: 2026-09-15

### 맥락

초점 링이 파랑 30% 한 겹이었다. **어디서도 보이지 않았다.**
문서 배경 위 1.53, 카카오 노랑 위 1.06, 네이버 초록 위 1.41 — WCAG 1.4.11 이 요구하는
3:1 을 넘는 조합이 검은 버튼뿐이었다. 다크 모드는 더 나빴다. 토큰이 모드가 하나뿐인
`Primitive` 에 있어서 **다크 값을 가질 수가 없는 구조**였다.

처음에는 모든 컴포넌트에 불투명 링을 두는 쪽으로 고쳤는데, 폼 요소에서 너무 두꺼웠다.
20×20 체크박스는 테두리가 이미 2px 인데 같은 색 링 2px 이 더 붙어 4px 한 덩어리로 보였다.

다시 보니 **폼 요소와 버튼은 애초에 사정이 다르다.**
입력은 초점일 때 테두리 색이 회색에서 파랑으로 바뀐다 — 그 변화가 이미 대비 3.68 을 낸다.
버튼은 테두리 색이 바뀌지 않는다 — 링 말고는 달라지는 것이 없다.

### 결정

**신호를 지는 것이 무엇인지로 갈린다.**

| | 신호 | 장식 |
| --- | --- | --- |
| 폼 요소 — Input · Textarea · Select · Checkbox · Radio | **테두리 색 변화** | 30% 링 `spread 2` |
| 테두리가 없는 것 — Button · Icon Button · Social Button · Con · List/Item · Toggle | **불투명 링 `spread 2`** | 30% 링 `spread 3` |

- **30% 링은 어느 경우에도 장식이다.** 대비를 지지 않는다
- 폼 요소는 **그림자 하나**다. 불투명 겹을 두지 않는다
- 체크박스·라디오도 초점일 때 테두리가 바뀌어야 한다. 그래서 `choice/box/border-focus` 를
  만들고 `field/border-focus` 와 같은 출처(`brand/500`)를 쓴다
- Toggle 은 폼 요소지만 **테두리가 없어** 불투명 링 쪽에 둔다
- **색은 검증 상태를 따라간다.** 중립은 `brand/*`, valid·invalid 는 테두리와 같은 색
- **초점 링은 focus 에만 둔다.** 체크박스·라디오의 pressed 에 링이 있었으나 뺐다 —
  누르는 중인 것과 초점이 있는 것은 다른 사실이고, 둘을 같은 표시로 알리면 구분이 사라진다
- 요소와 링 사이에 간격을 두지 않는다
- 토큰은 `Theme` 에 둔다. 모드별 값이 필요하기 때문이다

헤일로는 **별칭에 알파를 넣을 수 없어 모드별 직접값**이다.
짝이 되는 색이 바뀌면 따라오지 않으므로 손으로 맞춰야 한다 — 각 토큰 description 에 적어 둔다.

### 근거

반투명한 표시는 그 자체로 기준을 넘지 못한다. 뒤에 무엇이 오는지에 따라 합성색이 달라지고,
넘기려 알파를 올리면 투명해 보이지 않는다(파랑 기준 71% 가 필요하다).
그래서 **대비는 불투명한 것이 진다.** 문제는 "무엇이 불투명하냐"였고,
폼 요소에는 이미 그런 것이 있었다 — 테두리다.

없는 것을 새로 얹는 대신 **이미 있는 변화를 신호로 인정하면** 링은 얇아도 된다.
링을 장식으로 두면 요소 바깥으로 번지는 두께가 줄고, 작은 컨트롤에서 과해 보이지 않는다.

버튼에는 그 변화가 없다. 배경색은 hover·pressed 가 이미 쓰고 있어 초점에 내줄 수 없다.
그래서 버튼만 불투명 링이 신호를 진다.

색을 상태별로 가른 이유는 단순하다. 테두리는 빨강·초록인데 링만 파랑이면 둘이 어긋나 보인다.

### 뒤집으려면

`focus-ring()` 믹스인 한 곳과 Figma 의 이펙트를 **같이** 고친다.
폼 요소를 불투명 링으로 되돌리려면 테두리 색 변화를 빼도 되지만, 그러면
`choice/box/border-focus` 가 안 쓰이게 되므로 그것도 같이 정리한다.

**Figma 와 코드를 따로 고치지 않는다.** 한쪽만 바꾸면 검사가 잡지 못하는 종류의 어긋남이 생긴다 —
이펙트는 값이 들어 있어도 화면에 안 그려질 수 있어서, 스냅샷 대조로는 확인되지 않는다.

### 남은 어긋남

이 결정과 별개로, 이 작업 중에 발견했고 **아직 안 고친 것**이 둘 있다.

- Radio 는 검증 색 테두리를 쓰지 않는다. valid·invalid 의 모든 상태에서 회색이다(Checkbox 는 쓴다)
- Select 의 초점 테두리가 `brand/500` 을 직접 문다. Input·Textarea 는 `field/border-focus` 를 쓴다
