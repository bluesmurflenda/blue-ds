...
## ADR-038 · 초점 표시는 구조가 하나, 색만 검증 상태를 따른다
...
투명도를 유지한 채 한 겹으로 기준을 넘기려면 알파가 71% 여야 한다. 그쯤이면 투명해 보이지 않는다.

컴포넌트마다 두께와 방식도 갈려 있었다 — 어떤 것은 2px, 어떤 것은 3px,
검증 상태에서는 연초록·연빨강을 썼는데 그 색들도 기준에 한참 못 미쳤다.

### 결정

**두 겹으로 만든다. 구조는 모든 컴포넌트에서 같다.**

```
0 0 0 2px  <불투명 링>      ← 대비를 지는 쪽
0 0 0 3px  <반투명 30% 헤일로>  ← 장식. 대비를 지지 않는다
```

- **요소와 링 사이에 간격을 두지 않는다.** 링은 요소 바깥에 그려져 페이지 배경과 맞닿고,
  거기서 기준을 넘는다 — 라이트 4.95, 다크 7.05
- **색만 검증 상태를 따라간다.** 중립은 `brand/600`, valid·invalid 는 각각
  `field/border-valid` · `field/border-invalid` 를 별칭으로 따라간다.
  링과 테두리가 늘 같은 색이 되고, 테두리 하나만 바꾸면 둘이 같이 움직인다
- **두께는 상태와 무관하게 같다.** 색만 갈린다
- 토큰은 `Theme` 에 둔다. 모드별 값이 필요하기 때문이다

헤일로는 **별칭에 알파를 넣을 수 없어 모드별 직접값**이다.
짝이 되는 색이 바뀌면 헤일로는 따라오지 않으므로 손으로 맞춰야 한다 —
그 사실을 각 헤일로 토큰의 description 에 적어 둔다.

### 근거

반투명한 표시는 그 자체로 기준을 넘지 못한다. 뒤에 무엇이 오는지에 따라 합성색이 달라지고,
넘기려 알파를 올리면 투명해 보이지 않는다. **얇은 불투명 겹이 대비를 지고 반투명 겹은
그 바깥에 두면** 부드러운 인상을 잃지 않으면서 기준을 넘는다.

간격을 두면 어느 배경 위에서도 안전하지만 요소 바깥으로 그만큼 더 번진다.
간격 없이 가기로 한 대신, **링과 밝기가 비슷한 배경 위에서는 안쪽 경계가 흐려진다**는 것을
받아들인다. 바깥 경계가 페이지 배경과 맞닿아 기준을 넘으므로 표시가 사라지지는 않는다.

색을 상태별로 가른 이유는 단순하다. 테두리는 빨강·초록인데 링만 파랑이면
**둘이 어긋나 보인다.** 구조를 하나로 두는 것과 색이 상태를 알리는 것은 서로 충돌하지 않는다.

### 뒤집으려면

두께를 바꾸려면 `focus-ring()` 믹스인 한 곳과 Figma 의 이펙트 두 겹을 같이 고친다.
색을 중립 하나로 되돌리려면 `effect/focus-ring-valid` · `-invalid` 와 짝 헤일로를 지우고
해당 변형의 이펙트를 중립 짝으로 다시 물린다.
